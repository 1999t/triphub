...
> 覆盖登录、行程详情、热门榜单、用户画像与 AI 推荐等核心接口的异常用例，方便 QA 设计回归用例。
...
#### 1.2 `/user/auth/sendCode`

| 接口       | 场景                                 | 预期行为                                       |
|------------|--------------------------------------|------------------------------------------------|
| 发送验证码 | 正常手机号，频率在允许范围内         | `code = 0`，返回固定验证码 `"1234"`            |
| 发送验证码 | 手机号为空 / 非法格式                | `code = 1`，提示「手机号不能为空」或等价文案   |
| 发送验证码 | 同一 IP / 手机号请求过于频繁（限流） | `code = 1`，提示「请求过于频繁，请稍后再试」   |

#### 1.3 受保护接口（如 `/user/profile/me`）
...
| 接口               | 场景                        | 预期行为                                                  |
|--------------------|-----------------------------|-----------------------------------------------------------|
| 获取当前用户画像   | 用户未登录                 | `code = 1`，提示「未登录或 token 无效」                   |
| 获取当前用户画像   | 用户已登录且画像存在       | `code = 0`，返回画像 JSON                                 |
| 获取当前用户画像   | 用户已登录但画像尚未设置   | `code = 0`，`data` 为 `{}` 或空对象                       |
| 设置/更新用户画像 | Body JSON 非法 / 缺少必填字段 | `code = 1`，参数校验失败                                 |

#### 4.2 `/user/ai/trip-plan`

| 场景                                       | 预期行为                                                         |
|--------------------------------------------|------------------------------------------------------------------|
| 请求参数合法，AI 服务可用                  | `code = 0`，返回行程草稿与由 LLM 生成的解释文案                 |
| 未配置 AI Key / base-url 不可达 / 调用异常 | `code = 0`，行程正常写库，但 `explanation` 降级为本地拼接文案   |
| 用户画像 JSON 非法                          | `code = 0`，行程与解释正常返回，但解释文案不再依赖画像细节     |
| 调用频率过高触发限流                       | `code = 1`，提示「请求过于频繁，请稍后再试」                    |
