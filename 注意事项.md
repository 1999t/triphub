...
> 补充（2025-12-07）：在实践中，即便通过 `ant_path_matcher` 与各种 BeanPostProcessor 打补丁，Springfox 在 Spring Boot 2.7 上仍然存在不稳定因素。最终方案是**彻底迁移到 springdoc-openapi**：
> - 在 `triphub-server/pom.xml` 中移除 `knife4j-spring-boot-starter`，改用：
>   ```xml
>   <dependency>
...
>   </dependency>
>   ```
> - 新增 `OpenApiConfig`，使用 `io.swagger.v3.oas.models.OpenAPI` 与 `io.swagger.v3.oas.models.info.Info` 配置基础文档信息；
> - 之后访问 `http://localhost:8090/swagger-ui.html` 即可查看接口文档，不再依赖 Springfox / Knife4j；
> - 经验：**遇到老旧生态与新 Boot 版本频繁“打补丁”时，优先评估是否可以迁移到官方推荐的新方案，而不是在兼容层上反复造轮子。**
