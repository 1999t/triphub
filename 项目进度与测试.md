...
> 前置依赖：已完成用户登录流程（拿到 `{{userToken}}`），并且本地或远程环境已配置外部 LLM（如 OpenAI）API Key。
  ...
  - 在 `application.yml` 中检查：
    ...
    triphub:
      ai:
        ...
        api-key: ${TRIPHUB_AI_API_KEY:}
        model: gpt-4.1-mini
    ```
  - 在本机或容器环境中设置环境变量：
    ```bash
    export TRIPHUB_AI_API_KEY=你的真实APIKey
    ```
  - 重启 Triphub 服务，使配置生效。
- **数据库检查**
  - 确认已执行最新的 `db/init_triphub.sql`，存在 `user_profile` 表：
    ```sql
    DESC user_profile;
    ```

#### 2. 用户画像接口测试 `/user/profile`

- **2.1 获取当前用户画像（初始为空） `GET /user/profile`**
  - 请求：
...
- **2.2 设置/更新用户画像 `POST /user/profile`**
  - 请求：
    - URL：`{{baseUrl}}/user/profile`
    - Header：
      - `Content-Type: application/json`
  ...
  - 预期结果：
    ...
    - Body：
      ...
      ```
    - 再次调用 `GET /user/profile`，`data` 中能看到刚刚设置的 JSON。
    - 数据库中：
      ```sql
      SELECT * FROM user_profile WHERE user_id = <当前用户ID>;
      ```
      `profile_json` 字段为上述 JSON 串。

#### 3. AI 行程规划接口测试 `/user/ai/trip-plan`

- **请求**
  - URL：`{{baseUrl}}/user/ai/trip-plan`
  - Header：
    - `Content-Type: application/json`
    - `token: {{userToken}}`
  - Body(JSON 示例)：
  ...
  - Body：
    ```json
    {
      ...
      "data": {
        ...
        "explanation": "一段中文解释，说明为何该行程适合当前用户..."
      }
    }
    ```
    - 当 AI Key 配置正确且网络正常时：
      - `explanation` 为 LLM 生成的中文文案，通常会提到「美食」「预算」等画像信息。
    - 当 AI 调用失败或未配置 Key 时：
      - 仍然返回 `code = 0`；
      - `explanation` 由后端本地逻辑拼接的中文说明（例如包含「根据你的基础出行偏好」「预算区间」等字样）。
  - 数据库校验：
    ```sql
    SELECT * FROM trip WHERE id = <data.trip.id>;
    ```
    - 该记录的 `title` 前缀为「成都 3日...（AI推荐）」；
    - `user_id` 为当前用户 ID，`visibility = 0`。

#### 4. 个性化推荐行程接口测试 `/user/discover/recommend-trips`

> 该接口依赖热门行程 ZSet（`hot:trip`）以及用户画像。

- **4.1 准备热门行程数据**
  - 可复用前文「行程-创建」与「行程-详情」接口：
    1. 创建两条及以上公开行程（`visibility = 2`），目的地可以不同。
    2. 多次访问其中某几条 `/user/trip/{id}`，提升其浏览量，确保写入 `hot:trip`：
       ```shell
       ZREVRANGE hot:trip 0 10 WITHSCORES
       ```
       可看到若干 `tripId` 与对应分数。

- **4.2 推荐接口调用 `GET /user/discover/recommend-trips`**
  - 请求：
    - URL：`{{baseUrl}}/user/discover/recommend-trips?limit=5`
    - Header：
      - `token: {{userToken}}`
  - 预期结果：
    - HTTP 状态码：`200`
        ...
        "data": [
          {
            ...
            "destinationCity": "成都",
            "viewCount": 10,
            "reason": "This trip is popular in the current leaderboard and matches your interest in 美食. Destination: 成都."
          },
          {
            "tripId": 102,
            "title": "重庆三日夜景行",
            "destinationCity": "重庆",
            "viewCount": 8,
            "reason": "This trip is popular in the current leaderboard and matches your interest in 美食. Destination: 重庆."
          }
        ]
      }
      ```
    - 返回列表中的行程 ID 顺序与 `hot:trip` 中分数排序保持一致，只包含公开行程（`visibility = 2`）。
    - 当用户画像中存在 `tags` 且包含「美食」等标签时，`reason` 字段中会带上该标签名称；若没有画像或无标签，则仅说明「热门榜单」与目的地信息。

#### 5. 不同画像下行为对比（演示个性化效果）

1. 使用手机号 A 登录，设置画像为「美食 5 星」：
   - `POST /user/profile`，Body 中 `tags` 主打「美食」。
2. 访问 `/user/ai/trip-plan` 和 `/user/discover/recommend-trips`，记录返回的 `title` 与 `explanation` / `reason` 内容。
3. 使用手机号 B 登录，设置画像为「摄影 5 星」或「亲子/休闲」：
   - 再次调用同样的两个接口。
4. 对比两者：
   - `AiTripPlanVO.explanation` 中提到的偏好关键词不同；
   - 推荐列表中，`reason` 字段对「美食/摄影」的描述不同，从而体现「同一热门榜单 + 不同画像 → 不同解释 / 推荐侧重点」的 RAG 味道。
