...
- **使用场景**  
  ...
  项目中还保留了一条基于 Redis + Lua + MQ 的「限时秒杀」练习链路，更多用于高并发与一致性思路的 Demo，而非主业务。
...
- **Maven 多模块工程**
  ...
  - `triphub-pojo`：实体、DTO、VO 定义：
    - 实体：`User`、`Trip`、`TripDay`、`TripItem`、`UserProfile`、`TripFavorite` 等（另有 `SeckillActivity`、`Order` 等作为扩展 Demo）
    - DTO / VO：行程按天编辑 DTO、AI 行程请求 DTO、AI 行程返回 VO、推荐行程 VO、AI 助手 DTO 等
  - `triphub-server`：业务服务模块：
    - Controller：用户登录、行程、发现页、收藏、AI 等接口（另有独立的秒杀 Demo 接口）
...
- **数据库与初始化脚本**
  ...
  - 核心表：
    ...
    - （扩展 Demo）`seckill_activity` / `order`：限时秒杀活动与订单
...
- **6. 可选：限时秒杀旅游优惠（高并发 Demo）**
  - 为了练习高并发与最终一致性，项目中保留了一条基于 Redis + Lua + RabbitMQ 的秒杀 Demo 链路，包括：
    - 活动配置与库存预热；
    - Lua 预检库存与一人一单；
    - 异步下单与对账自愈任务；
  - 这部分更适合作为技术加分项按需介绍，主线业务仍以行程管理 / 收藏画像 / AI 推荐为主。
...
- **1. 可观测性与监控模块**
  ...
  - **基于 Redis 的接口限流实现**：
    ...
    - 对秒杀下单接口 `/user/seckill/{activityId}`（作为高并发 Demo）：
...
- **基础技术栈**
  ...
  - MySQL（业务数据持久化）
  - Redis（缓存、ZSet 排行榜、收藏统计与少量高并发 Demo）
  - RabbitMQ（秒杀链路 Demo 的异步下单）
  - Redisson（分布式锁）
  - JWT（用户端鉴权）
...
- **3. 核心业务模块关系（行程 / 收藏 / 画像 / AI）**

```mermaid
graph LR
  subgraph 用户行为
    U[用户] -->|创建/编辑| TripAPI[/Trip 接口/]
    U -->|收藏/取消收藏| FavAPI[/Favorite 接口/]
    U -->|调用 AI 行程规划/助手| AIApi[/AI 接口/]
  end

  TripAPI --> TripSvc[TripService]
  FavAPI --> FavSvc[TripFavoriteService]
  AIApi --> AISvc[AiController]

  TripSvc --> TripTable[(trip / trip_day / trip_item)]
  FavSvc --> FavTable[(trip_favorite)]

  FavSvc --> ProfileSvc[UserProfileService]
  ProfileSvc --> ProfileTable[(user_profile)]

  AISvc -->|读取画像+收藏统计| ProfileSvc
  AISvc -->|写入新 Trip 草稿| TripSvc
  AISvc -->|调用 LLM| LLM[AiClient -> 外部模型]

  AISvc --> RedisIdem[(Redis<br/>AI 幂等缓存)]
  AIApi --> RateLimiter[SimpleRateLimiter<br/>按用户限流] --> RedisRate[(Redis 计数)]
```
