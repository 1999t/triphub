...
- **2. 接口安全与限流模块**
  - 基于 JWT 的用户鉴权 + 资源级权限校验：所有 `/user/**` 接口均依赖拦截器解析 token 并注入 `BaseContext`，敏感接口会校验当前用户是否有权访问目标行程或活动。
  - 对易被刷接口（如验证码发送、秒杀下单）设计基于 Redis 的限流方案：按 IP / 手机号 / 用户 ID 维度限制单位时间内的调用次数，防止恶意脚本冲击。
  - 在 `docs/developer/api_error_matrix.md` 中补充了未登录、越权访问、限流触发等异常场景的预期行为，便于在安全相关问题上进行系统性回归。

- **3. 一致性与对账模块**
  - 秒杀链路采用「Redis 库存 + Lua 预检 + MQ 异步下单 + 数据库库存扣减」的最终一致性模型，避免在高并发下频繁锁表。
  - 设计定期对账思路：周期性对比 `seckill:stock:{id}` / `seckill:order:{id}` 与 `seckill_activity.stock`、`order` 表记录数，一旦发现差异即输出详细日志，作为后续报警与补偿的基础。
  - 对热门榜单（`hot:trip`、`hot:dest`）也给出从数据库或埋点数据重建 ZSet 的流程说明，保证在 Redis 故障或清空后可以按需恢复。

- **4. 测试与质量保障模块**
  - 提供完整的 Postman 集合 `triphub-auth.postman_collection.json`，覆盖登录、行程管理、按天编辑、热门榜单、秒杀、用户画像、AI 推荐等核心接口，并通过 Tests 脚本自动串联 token、tripId、tripItemId 等上下文变量。
  - 在 `docs/` 下按模块拆分接口说明与测试步骤（auth / trip / discover / seckill / ai），形成结构化的手工测试用例库，方便面试演示和团队协作。
  - 规划为缓存组件（如 `CacheClient`）、ID 生成器（`RedisIdWorker`）以及秒杀服务编写单元测试与端到端集成测试（基于 SpringBootTest + Testcontainers），使项目具备向自动化回归演进的空间。

- **5. 工程化与部署支持模块**
  - 配置管理：通过 `application.yml` + 环境变量的方式管理数据库、Redis、RabbitMQ、AI Key 等敏感配置，便于在本地 / 测试 / 线上环境间切换。
  - 初始化与运维脚本：提供 `db/init_triphub.sql` 与 `script/init_seckill.sh`，一键完成核心库表与秒杀活动、库存初始化，降低环境搭建成本。
  - 容器化与多环境支持的预留：项目结构与配置命名已按「dev / prod」区分，易于后续引入 Dockerfile 与 docker-compose，将 MySQL、Redis、RabbitMQ 与 TripHub 服务打包为一键启动的本地开发环境。
